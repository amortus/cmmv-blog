    ...
    <div class="flex flex-col min-h-screen bg-gray-100">
        ...
        <header class="bg-[#001E62] sticky top-0 z-50 shadow-md">
            <div class="max-w-[1200px] mx-auto px-4">
                    ...
                    <nav class="hidden md:flex items-center space-x-3">
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <div v-if="mainNavCategories.childrenMap[category.id]" class="relative">
                                ...
                                >
                                    <a v-for="child in mainNavCategories.childrenMap[category.id]" :key="child.id"
                                        ...
                                        class="block text-white hover:bg-[#00378F] hover:text-[#ffcc00] px-4 py-3 text-lg transition-colors"
                                    >
                                        {{ child.name }}
                                    </a>
                                </div>
                            ...
                            <a v-else
                                ...
                                class="text-white hover:bg-[#00378F] dark:hover:bg-[#001E62] hover:text-[#ffcc00] px-3 py-2 rounded text-lg whitespace-nowrap transition-colors"
                            >
                                {{ category.name }}
                            </a>
                        </template>
                ...
                <!-- Mobile Menu -->
                <div v-show="mobileMenuOpen" class="md:hidden py-3 border-t border-[#00378F]">
                    <div class="flex flex-col gap-1">
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <div v-if="mainNavCategories.childrenMap[category.id]" class="w-full">
                                ...
                                <div v-show="openDropdowns[category.id]" class="pl-4 py-1 bg-[#00378F] rounded mt-1">
                                    <a
                                        v-for="child in mainNavCategories.childrenMap[category.id]"
                                        :key="child.id"
                                        :href="`/category/${child.slug}`"
                                        class="block px-4 py-2 text-lg text-white hover:bg-[#001E62] hover:text-[#ffcc00] rounded"
                                    >
                                        {{ child.name }}
                                    </a>
                                </div>
                            ...
                            <a
                                v-else
                                :href="`/category/${category.slug}`"
                                class="block text-white hover:bg-[#00378F] dark:hover:bg-[#001E62] hover:text-[#ffcc00] rounded px-3 py-2 text-lg"
                            >
                                {{ category.name }}
                            </a>
                        </template>
                    ...
                    <div class="flex justify-center space-x-6 mt-3 pt-3 border-t border-[#00378F]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-1 gap-8">
                    <div>
                        <h3 class="text-lg font-bold mb-4 relative pb-3">
                            Sobre o Aquiesportes
                            <span class="absolute bottom-0 left-0 w-10 h-1 bg-[#0091FF]"></span>
                        </h3>
                        <p class="text-gray-300 text-sm mb-4">
                            Portal especializado em notícias, reviews e conteúdo sobre esportes.
