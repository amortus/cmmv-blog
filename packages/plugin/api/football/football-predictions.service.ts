...
export class FootballPredictionsService {
    ...
    async fetchFromApi(
        ...
        try {
            ...
            const predictions: FootballPrediction[] = data.predictions.map((prediction: any) => ({
                competition: competition,
            ...
            this.logger.error('Error fetching from API:', error);
            throw error;
        }
    }
    ...
    async getPredictionsWithCache(
        ...
        try {
            const shouldUpdate = this.shouldUpdateCache(competition, intervalHours);
            ...
            return this.getCachedPredictions(competition, limit);

        } catch (error) {
            this.logger.error('Error in getPredictionsWithCache:', error);
            return [];
        }
    }
